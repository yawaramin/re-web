...
let s = "ReWeb.Cache", [
  Alcotest.test_case "make, add, find_opt" `Quick begin fun () ->
  ...
  Alcotest.test_case "make, add, remove, find_opt" `Quick begin fun () ->
    let cache = Cache.make () in
    Lwt_main.run begin
      let open Lwt.Syntax in
      let+ result = Cache.access cache begin fun table ->
  ...
  Alcotest.test_case "concurrent operations are serialized" `Slow begin fun () ->
    let cache = Cache.make () in
    Lwt_main.run begin
      let open Lwt.Syntax in
